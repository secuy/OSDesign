...
public class ProcessUI extends JFrame{
	...
	public static void setDeviceMessage() {  //设置设备信息
		if(ProcessSchedule.getBlockQ1().size()>0) {
			deviceText[0].setText("进程"+ProcessSchedule.getBlockQ1().get(0).getPcb().getPro_ID());
		} else {
			deviceText[0].setText("");
		}
		if(ProcessSchedule.getBlockQ2().size()>0) {
			deviceText[1].setText("进程"+ProcessSchedule.getBlockQ2().get(0).getPcb().getPro_ID());
		} else {
			deviceText[1].setText("");
		}
		if(ProcessSchedule.getBlockQ3().size()>0) {
			deviceText[2].setText("进程"+ProcessSchedule.getBlockQ3().get(0).getPcb().getPro_ID());
		} else {
